...
show_help() {
    ...
    echo
    echo "步骤: 1/cn=archlinuxcn  2/im=IBus原生  3/wb=WorkBuddy  4/backkey=GPD背键  5/decky  6/games=游戏  7/dsh=DeepSeek Harness  8/clean=rootfs瘦身  9/tdp=TDP控制(SimpleDeckyTDP)  10/ntp=换境内NTP(加速开机)  11/gpu=GPU加速建议(DLSS/FSR)  12/selfheal=升级后自愈服务"
    ...
    echo "  x11 模式需配套: IBUS_XWAYLAND=1 sudo bash $SCRIPT_NAME 2  (补 GTK_IM_MODULE=ibus)"
...
verify_step() {
    case "$1" in
        ...
        setup_im)      pacman -Qq ibus >/dev/null 2>&1 && [ -x /usr/bin/ibus-daemon ] && \
                       { pacman -Qq ibus-libpinyin >/dev/null 2>&1 || \
                         pacman -Qq ibus-rime >/dev/null 2>&1 || \
                         pacman -Qq ibus-pinyin >/dev/null 2>&1; } ;;
...
setup_wb() {
    ...
    if [ -f "$WRAPPER" ]; then
        ...
        if [ "$WB_IME_MODE" = "x11" ]; then
            info "x11 模式需 GTK_IM_MODULE=ibus: 已由第[2]步 IBUS_XWAYLAND 选项负责(见 setup_im)"
        fi
    else
...
setup_tdp() {
    ...
    if [ -d "$PLUGIN_DIR/$PLUGIN_NAME" ]; then
        local BAK="$PLUGIN_DIR/${PLUGIN_NAME}.bak.$(date +%m%d-%H%M%S)"
...
setup_selfheal() {
    ...
    local MAIN_ABS="$(cd "$(dirname "$0")" 2>/dev/null && pwd)/steamos-setup.sh"

    # ── 1. 安装自愈脚本(若备份包里没有, 就内嵌生成) ──
    local SH_SRC="$(cd "$(dirname "$0")" 2>/dev/null && pwd)/self-heal-after-upgrade.sh"
    mkdir -p "$SH_DIR"
    if [ -f "$SH_SRC" ]; then
...
show_status() {
    ...
    pacman -Qq workbuddy >/dev/null 2>&1 && {
        ...
        local WB_MARK WB_MODE
        if grep -q -- "--wayland-text-input-version=3" /usr/bin/workbuddy 2>/dev/null; then
            WB_MARK="--wayland-text-input-version=3"; WB_MODE="text-input-v3(非默认)"
...
if [ "$AFTER_UPGRADE" -eq 1 ]; then
    ...
    if [ -n "$_avail" ] && [ "$_avail" -lt 600 ]; then
        warn "rootfs 仅剩 ${_avail}MB。恢复需要重装 pacman 包(IBus 等), 建议先腾空间:"
